...
import type { LogosSheet, LogosImplSheet } from "./codegen";
...
export function validateRunnables(impl: LogosImplSheet, declared: DeclaredRunnable[]): ReadyRunnable[] {
  const defined = topLevelFunctionNames(impl);
  return declared
    .filter((r) => defined.has(r.name))
    .map((r) => ({ ...r, ready: true as const }));
}
